...
# Rotate the whole tensegrity 90 degrees to get to default orientation with two long rods poin
...
for angleIdx0 in range (sectionNum):
    for angleIdx1 in range (sectionNum):
        ...
        pWall_p = Vec3(np.min(initPos_p[:,0]),0,0) #Set the wall so that contact starts right at time 0
        
        prop_guard_ODE =prop_guard_ode(prop_guard)
        sol_p = solve_ivp(prop_guard_ODE.ode_ivp_wall, t_span, P_p, method='Radau',args=(nWall, kWall, pWall_p))
        tHist_p = sol_p.t
        Ps_p= sol_p.y
        ...
        pWall_t = Vec3(np.min(initPos_t[:,0]),0,0) #Set the wall so that contact starts right at time 0

        tensegrity_ODE =tensegrity_ode(tensegrity)
        sol_t = solve_ivp(tensegrity_ODE.ode_ivp_wall, t_span, P_t, method='Radau',args=(nWall, kWall, pWall_t))
        tHist_t = sol_t.t
        Ps_t= sol_t.y
